...
import { type CapacitorListenerHandle, firebaseMessagingPlugin, nativeShellVersion } from './native-shell';
...
async function registerPushDevice(token: string): Promise<void> {
  ...
  try {
    // The shell's marketing version, which the backend records on the device row
    // and picks the push payload shape by: an Android shell from 1.0.0 gets
    // DATA-ONLY pushes it must render itself (`AndroidDataOnlyPushFormatter.SINCE`),
    // so this must only ever be reported by a bundle shipped inside a shell that
    // can. Null when the shell cannot say — the backend then nulls the column,
    // which keeps a reinstalled older build on the payload it can show.
    const appVersion = await nativeShellVersion();
