 ...
 *
 * The device figures come through `deviceAllocation`, the reader the billing
 * page's own counter uses: it selects the count and the committed quantity and
 * no price, which is what lets this query spread it — and what keeps the two
 * pages from disagreeing on whether a workspace is over its limit.
 */
 ...
 *        billing page uses — see `device-allocation.ts`).
...
export function UsageView() {
  ...
  const device = subscription ? deviceAllocation(subscription) : null;
  ...
  const devicePerUse = device?.isPayg ?? false;
  const devicesUsed = device?.used ?? 0;
  const deviceLimit = device?.allocation ?? 0;
  ...
  const deviceOverLimit = device?.overLimit ?? false;
