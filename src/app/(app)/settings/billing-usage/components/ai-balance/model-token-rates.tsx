 ...
 */
export function ModelTokenRatesPopover({ autoTopUpEnabled, className }: ModelTokenRatesProps & { className?: string }) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" sideOffset={8} className="border-0 bg-transparent p-0 shadow-none">
        <ModelTokenRates autoTopUpEnabled={autoTopUpEnabled} />
      </DropdownMenuContent>
    </DropdownMenu>
  );
}
 ...
 */
export function ModelTokenRates({ autoTopUpEnabled }: ModelTokenRatesProps) {
  return (
...
function AutoTopUpLine({ enabled }: { enabled: boolean }) {
  ...
  return (
    ...
    >
      ...
      <span className="flex-1">{enabled ? 'Auto Top Up Enabled' : 'Auto Top Up Disabled'}</span>
    </div>
  );
}
