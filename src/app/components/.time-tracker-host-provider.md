...
|---|---|
| `TimeTrackerHostProvider` | Public entry point; ALWAYS mounts `TimeTrackerHost`, passing `enabled` down |
...
```mermaid
graph TD
    A[TimeTrackerHostProvider] -->|enabled prop| B[TimeTrackerHost]
...
- **Always mounted**: `enabled` is passed to the host, never used to decide whether to render it. Swapping between `<TimeTrackerHost>` and a fragment changes the element type when the flag answers mid-boot, and React remounts everything below — which here is the whole app shell and the page inside it. With `enabled` false the host mounts but issues no requests (hydrators and modals unrendered, option queries disabled) and `TimeTrackerProvider` supplies no context, so consumers behave as if it were absent.
